    ...
    public class MartilleroDAL : AbstractDAL<Martillero>
    {
        public Martillero ObtenerPorUsername(string username)
        {
            var tabla = _acceso.Leer(
                "SELECT Id, Nombre, Username, PasswordHash, IntentosFallidos, BloqueadoHasta " +
        ...
        public override void Guardar(Martillero m)
        {
            _acceso.Escribir(
                @"IF EXISTS (SELECT 1 FROM Martilleros WHERE Username=@u)
                      UPDATE Martilleros SET Nombre=@n, PasswordHash=@ph, IntentosFallidos=@if, BloqueadoHasta=@bh WHERE Username=@u
                  ELSE
                      INSERT INTO Martilleros (Nombre, Username, PasswordHash, IntentosFallidos, BloqueadoHasta)
                      VALUES (@n, @u, @ph, @if, @bh)",
                new SqlParameter[]
                {
                    new SqlParameter("@n",  m.Nombre  ?? ""),
                    new SqlParameter("@u",  m.Username),
        ...
        {
            var tabla = _acceso.Leer(
                "SELECT Id, Nombre, Username, PasswordHash, IntentosFallidos, BloqueadoHasta FROM Martilleros");
        ...
        {
            var tabla = _acceso.Leer(
                "SELECT Id, Nombre, Username, PasswordHash, IntentosFallidos, BloqueadoHasta FROM Martilleros WHERE Id=@id",
        ...
        {
            Id               = (int)f["Id"],
            Nombre           = f["Nombre"] == DBNull.Value ? "" : (string)f["Nombre"],
